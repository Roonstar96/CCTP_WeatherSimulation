...
public class CloudManager : MonoBehaviour
{
    ...
    public LocalWeatherManager weather;
    ...
    private void CurrentWaterStored()
    {
        ...
        if (_isStoring)
        {
            Mathf.Round(_waterStored += (weather.Evaporation / 10) * Time.deltaTime);
            //Debug.Log("WaterStored: " + _waterStored);
            if (weather.Tempurature <= 2)
            {
    ...
    private void CalculateRainVariables()
    {
        ...
        else if (weather.Tempurature > -5 && weather.Tempurature <= 2 )
        {
            float snowMultiplier = (Mathf.Abs(weather.Tempurature) / 100) + 1;

            Mathf.Round(_intensity = ((_waterStored / _cloudSize) / Mathf.Abs(weather.Tempurature) / 5));
            Mathf.Round(_duration = ((_waterStored / Mathf.Abs(weather.Tempurature)) * _cloudSize));
        ...
        else
        {
            if (_cloudSize == 100)
            {
                Mathf.Round(_intensity = ((_waterStored / _cloudSize) / Mathf.Abs(weather.Tempurature) / 10));
                Mathf.Round(_duration = ((_waterStored / Mathf.Abs(weather.Tempurature)) * _cloudSize));
                _isRaining = true;
                _isStoring = false;
            }
            else
            {
                ...
                Mathf.Round(_timeTillRain = (((_waterStored + _cloudSize) * Mathf.Abs(weather.Tempurature)) / 60));
                Mathf.Round(_intensity = ((_waterStored / _cloudSize) / (Mathf.Abs(weather.Tempurature) / 10)));
                Mathf.Round(_duration = ((_waterStored / Mathf.Abs(weather.Tempurature)) * _cloudSize));
                _isCounting = true;
            }
    ...
    { 
        if (weather.Tempurature < 2 && weather.Tempurature > -5)
        {
    ...
    private void CloudIsSnowing()
    {
        ...
        if(weather.Tempurature <= 2 && weather.Tempurature >= 0)
        {
            _main.startSpeed = 1;
        ...
        {
            _main.startSpeed = Mathf.Abs(weather.Tempurature);
            _main.startLifetime = 20 - Mathf.Abs(weather.Tempurature);
        }
